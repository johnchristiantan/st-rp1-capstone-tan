...
export default function ServiceCard() {
    ...
    const handleAddService = () => {
        ...
        const lastServiceCardID = serviceCards.length > 0 ? serviceCards[serviceCards.length - 1].id : 0

        // Check if it is the first card
        if (lastServiceCardID === 0 && serviceCardIndex === 0) {
            setServiceCards([{id: serviceCardIndex, card:  <AvailedServiceLists serviceCardIndex={serviceCardIndex} />}])
            return setServiceCardIndex(serviceCardIndex + 1)
        }

        const newServiceCard = { id: serviceCardIndex, card:  <AvailedServiceLists serviceCardIndex={serviceCardIndex} />}
